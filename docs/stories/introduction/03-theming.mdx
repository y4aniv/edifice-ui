...
We use [Edifice Bootstrap](https://github.com/opendigitaleducation/edifice-bootstrap) to theme our components and applications.
...
A watcher is already configured.

```
pnpm watch:react
```

This watcher will listen all changes in `edifice-bootstrap` and copy build files to Storybook documentation. So you can see any CSS you are doing.
...
⚠️ If one Bootstrap's component doesn't have CSS Variables, we add them to override components (E.g: `_form-control.scss`)
