...
export class SessionFramework implements ISessionFramework {
    ...
    public login(email: string, password: string, rememberMe?: boolean, secureLocation?: boolean): Promise<void> {
        ...
        return transport.http.post<void>('/auth/login', data, {
    ...
    logout(): Promise<void> {
        return transport.http.get<void>('/auth/logout')
            .finally(() => {
