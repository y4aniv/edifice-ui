...
export class ExplorerContext implements IExplorerContext {
  ...
  getResources(): Promise<GetResourcesResult> {
    const parameters: ISearchParameters = this.duplicateSearchParameters();
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.SEARCH, parameters)
      .then((ar) => {
        let result = ar as GetResourcesResult;
        // TODO data sanity check
        if (!result) throw new Error(ERROR_CODE.UNKNOWN);
        // Publish this results for listeners.
  ...
  createFolder(
    ...
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.CREATE, parameters)
      .then((ar) => {
  ...
  updateFolder(
    ...
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.UPD_PROPS, parameters)
      .then((ar) => {
  ...
  copy(targetId: string, resourceIds: ID[], folderIds: ID[]): Promise<void> {
    ...
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.COPY, parameters)
      .then((ar) => {
        // void
      });
  }
  move(targetId: string, resourceIds: ID[], folderIds: ID[]): Promise<void> {
    ...
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.MOVE, parameters)
      .then((ar) => {
        // void
      });
  }
  delete(resourceIds: string[], folderIds: string[]): Promise<void> {
    ...
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.DELETE, parameters)
      .then((ar) => {
        // void
      });
  }
  trash(
    ...
    return this.bus
      .publish(RESOURCE.FOLDER, ACTION.TRASH, parameters)
      .then((ar) => {
        // void
      });
  }
  manageProperties(
    ...
    return this.bus.publish(resourceType, ACTION.MANAGE, params).then((ar) => {
  ...
  updateProperties(
    ...
    const params: UpdatePropertiesParameters = {
      resources: resources,
      props: props,
    };
    return this.bus
      .publish(resourceType, ACTION.UPD_PROPS, params)
      .then((ar) => {
  ...
  publish(
    ...
    return this.bus
      .publish(resourceType, ACTION.PUBLISH, parameters)
      .then((ar) => {
