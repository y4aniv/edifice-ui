...
export class Http implements IHttp {
    ...
    private toCdnUrl( url:string ) {
        const CDN_DOMAIN:string|undefined = ConfigurationFrameworkFactory.instance().Platform.cdnDomain;
    ...
    get<R=any>(url: string, params?: IHttpParams): Promise<R> {
        return this.axios.get<R>(this.toCdnUrl(url), this.toAxiosConfig(params))
...
*/
    ...
    getScript<R=any>(url: string, params?: IHttpParams, variableName?:string): Promise<R> {
        ...
        return this.axios.get<string>(this.toCdnUrl(url), p)
            .then( r => this.mapAxiosResponse(r,params) )
            .then( r => {
                try {
