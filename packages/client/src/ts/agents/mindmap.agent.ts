...
console.log("Mindmap agent loading....");

class MindmapAgent extends AbstractBusAgent {
  constructor() {
    super(RESOURCE.MINDMAP);
    this.registerHandlers();
    console.log("Mindmap agent initialized!");
  }

  protected ctx: IContext | null = null;
  protected http: IHttp =
    TransportFrameworkFactory.instance().newHttpInstance();
  ...
  public registerHandlers(): void {
    this.setHandler(ACTION.OPEN, this.openMindmap as unknown as IHandler);
    this.setHandler(ACTION.CREATE, this.createMindmap as unknown as IHandler);
    this.setHandler(ACTION.MANAGE, this.onManage as unknown as IHandler);
    this.setHandler(
      ACTION.UPD_PROPS,
      this.onUpdateProps as unknown as IHandler,
    );
  ...
  onPrint(parameters: PrintParameters): void {
    window.open(
      `/blog/print/blog#/print/${parameters.resourceId}?comments=${
        parameters.withComments || true
      }`,
      "_blank",
    );
  }
  ...
  onUpdateProps(
    parameters: UpdatePropertiesParameters,
...
export const factory: AgentFactory = () => new MindmapAgent();
