...
export class Idiom implements IIdiom {
    ...
    private loadBundle( lang:string, path:string, callback?:AddBundleCallback): void {
        ...
        if (oldPromise) {
            ...
            transport.http.get<string>( path, {headers: params}).then( response => {
    ...
    addAllTranslations(folders:string[]):Promise<void> {
        if( folders && folders.length>0 ) {
            ...
            .then( lang => Promise.all( folders.map(folder => this.loadBundlePromise(lang, folder + '/' + lang + '.json')) ) )
            .then( () => {} );
        } else {
