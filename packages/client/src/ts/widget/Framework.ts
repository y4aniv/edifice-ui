    ...
    private async applyUserPrefs( prefs:IWidgetUserPrefs ):Promise<void> {
        ...
        skin.listSkins().then( skins => {
            ...
            this._widgets = this._widgets.sort((a, b) => {
                const translatedA = lang.translate(`timeline.settings.${a.platformConf.name}`).toLowerCase();
                const translatedB = lang.translate(`timeline.settings.${b.platformConf.name}`).toLowerCase();
